...
func createMsgwithTemplateSet(isIPv6 bool) *entities.Message {
	...
	ie10 := entities.NewDateTimeMillisecondsInfoElement(entities.NewInfoElement("flowEndMilliseconds", 153, 15, 0, 8), 0)
...
func createDataMsgForSrc(t *testing.T, isIPv6 bool, isIntraNode bool, isUpdatedRecord bool, isToExternal bool, isEgressDeny bool) *entities.Message {
	...
	if !isUpdatedRecord {
		ie10 = entities.NewDateTimeMillisecondsInfoElement(tmpFlowEndMillisecs, uint64(1*SecondToMilliSecond))
		...
		ie10 = entities.NewDateTimeMillisecondsInfoElement(tmpFlowEndMillisecs, uint64(10*SecondToMilliSecond))
...
func createDataMsgForDst(t *testing.T, isIPv6 bool, isIntraNode bool, isUpdatedRecord bool, isIngressReject bool, isIngressDrop bool) *entities.Message {
	...
	if !isUpdatedRecord {
		flowEndTime = uint64(1 * SecondToMilliSecond)
		flowEndReason = registry.ActiveTimeoutReason
		tcpState = "ESTABLISHED"
	} else {
		flowEndTime = uint64(10 * SecondToMilliSecond)
...
func assertElementMap(t *testing.T, record map[string]interface{}, ipv6 bool) {
	...
	assert.Equal(t, uint64(1*SecondToMilliSecond), record["flowEndMilliseconds"])
	assert.Equal(t, uint64(1*SecondToMilliSecond), record["flowEndMillisecondsFromSourceNode"])
	assert.Equal(t, uint64(1*SecondToMilliSecond), record["flowEndMillisecondsFromDestinationNode"])
