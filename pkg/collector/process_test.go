...
func TestUnknownInformationElement(t *testing.T) {
	...
	for _, mode := range []DecodingMode{DecodingModeStrict, DecodingModeLenientKeepUnknown, DecodingModeLenientDropUnknown} {
		t.Run(string(mode), func(t *testing.T) {
			...
			unknownIE := entities.NewInfoElement("foo", unknownID, entities.Unsigned32, registry.AntreaEnterpriseID, 4)
			...
			if mode == DecodingModeLenientKeepUnknown {
				...
				require.Equal(t, entities.NewInfoElement("", unknownID, entities.OctetArray, registry.AntreaEnterpriseID, 4), ieWithValue.GetInfoElement())
